...
def main():
    novoNinho_file = open("saidas/novo_ninho_bcSentimento.txt", "w")  # Gravação
    melhorNinho_file = open("saidas/melhor_bcSentimento.txt", "w")  # Gravação
    medias_file = open("saidas/media_bcSentimento.txt", "w")  # Gravação
    ...
    for epoca in range(busca_Cuco_Genetico.maxGeracoes):
        ...
        #Executa a cada x gerações - as operações são pesadas
         #Se ele executou os operadores genéticos, não precisa executar a AbandonaPa
